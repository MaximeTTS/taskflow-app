...
/**
 * `projects` et `tasks` renvoient desormais une page (`items` + `totalCount`
 * + `hasMore`) et non plus une liste nue : sans pagination, un compte charge
 * remontait la totalite de ses projets et de leurs taches a chaque affichage
 * du tableau de bord.
 *
 * La reponse est remise a plat juste apres la requete, ce qui laisse tout le
 * reste de la page lire `project.tasks` comme un tableau.
 */
...
export default function DashboardPage() {
  ...
  // La session vit dans un cookie `httpOnly` : le client ne peut plus la lire,
  // il doit la demander. On interroge donc le serveur avant de decider s'il
  // faut rediriger — un test sur `localStorage` ne dirait plus rien.
  useEffect(() => {
    void (async () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const page = data as {
        projects: { items: (Omit<Project, 'tasks'> & { tasks: { items: Task[] } })[] };
      };
      setProjects(page.projects.items.map((p) => ({ ...p, tasks: p.tasks.items })));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
